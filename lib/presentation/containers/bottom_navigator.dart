...
class BotonNavegacion extends StatefulWidget {
  // Define la clase BotonNavegacion que extiende StatefulWidget.
  final ValueChanged<int>
      onTabSelected; // Añade un callback que se ejecutará cuando se seleccione una pestaña.

  const BotonNavegacion({required this.onTabSelected, Key? key})
      : super(key: key);

  @override
  State<BotonNavegacion> createState() =>
      BotonNavegacionState(); // Crea un estado para el widget de navegación.
}

class BotonNavegacionState extends State<BotonNavegacion> {
  // Define el estado del widget de navegación.
      ...
      onTap: (int index) {
        ...
        setState(() {
          ...
          widget.onTabSelected(
              index); // Notifica al widget padre (MainScreen) sobre el cambio de pestaña.
        });
      },
      iconSize: 20, // Tamaño de los íconos.
      backgroundColor: color.primary, // Establece un color de fondo para la barra de navegación.
      unselectedItemColor: color.secondary, // Establece un color para los íconos no seleccionados.
      selectedItemColor:
